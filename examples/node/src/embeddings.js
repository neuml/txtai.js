...
const run = async () => {
    try {
        ...
        let documents = sections.map((text, index) => ({id: index, text: text}));
        ...
        for (let query of ["feel good story", "climate change", "health", "war", "wildlife", "asia", "north america", "dishonest junk"]) {
            let results = await embeddings.similarity(query, sections);
            let argmax = results.indexOf(Math.max(...results));
            console.log(sprintf("%-20s %s", query, sections[argmax]))
        }
